...
class RestApiJobManagement extends MockJobManagement {
    ...
    String getConfig(String jobName) throws JobConfigurationNotFoundException {
        assert jobName != null || jobName == ""

        String xml = fetchExistingXml(jobName)

        if (!xml) {throw new JobConfigurationNotFoundException(jobName)}
    ...
    InputStream streamFileInWorkspace(String filePath) throws IOException {
        ...
        return new File(filePath).newInputStream()
    }
    ...
    private setCrumbHeader() {
        ...
        HttpResponseDecorator resp = restClient.get(path: 'crumbIssuer/api/xml') as HttpResponseDecorator
